...
class multiInfoGraph:
    ...
    def refresh(self, ckpts):
        ...
        for ckpt, state in ckpts.items():
            if not ckpt in self._testName: continue
    ...
    def check(self):
        unCheckIdList = np.where(np.asarray(self._D_mat.sum(axis=1)) < 0.25)[0]

        fuzzyIdList = np.where(np.asarray(self._fuzzy_mat.sum(axis=1)) > 0.75)[0]
        ...
        collisionList = [sId for sId, fId in indices if (fId, sId) in indices]
        ...
        collisionList = []
        for fId, fNodeId in enumerate(self._faultIdList):
            self._nodes[fNodeId]["detectable"] = fId not in unCheckIdList
            self._nodes[fNodeId]["fuzzible"] = fId in fuzzyIdList
            self._nodes[fNodeId]["collision"] = fId in collisionList
