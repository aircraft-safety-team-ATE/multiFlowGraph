...
app = Flask(__name__, template_folder=os.path.join(r".", "templates"), static_folder=os.path.join(r".", "static"), static_url_path='/static')
...
@ app.route('/')
def init():
    return render_template("mfg-panel.html")
...
def result_analyse_main():
    graphStruct = json.loads(request.form.get("graphStruct"))
    values = pd.read_csv(request.files.get("dataFile"), header=0, encoding="gbk").T.to_dict().values()
    MIG = multiInfoGraph(graphStruct)
    MIG.refresh(values)
